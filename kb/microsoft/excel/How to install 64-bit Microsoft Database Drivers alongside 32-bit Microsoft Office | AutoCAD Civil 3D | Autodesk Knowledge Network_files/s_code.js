...
if (host == "knowledge.autodesk.com") {
    s_account = "autodesk-new-gl";
} else {
    s_account = "autodeskdevsite";
}

//s_account = "autodeskdevsite, autodesk-new-gl";
...
s.usePlugins = true; // s_doPlugins will be used
...
s.linkDownloadFileTypes = "avi,bin,dll,dmg,doc,docx,dwf,dwg,exe,gz,hqx,mov,mp3,mp4,mpg,msi,msp,pdf,ppt,pptx,rar,tgz,wav,wmv,xls,xlsx,zip";
s.linkInternalFilters = "javascript:,autodeskstg.prod.acquia-sites.com,knowledge.autodesk.com,knowledge-staging.autodesk.com,knowledge-dev.autodesk.com,knowledge-int.autodesk.com,knowledge-loc.autodesk.com";


var s_doPlugins = function(s)
{ // SiteCatalyst Do Plug-ins function
    if (!s.eVar1)
    {// mktvar004 parameter
        s.eVar1 = s.getQueryParam('mktvar004');
        s.eVar1 = s.getValOnce(s.eVar1, 's_mktvar004', 0);
    }

    if (!s.campaign)
    { // campaign
        s.campaign = s.getQueryParam('mktvar002');
        s.campaign = s.getValOnce(s.campaign, 'adsk_s_v0', 0);
    }

    if (!s.eVar5)
    {// Marketo LeadID
        s.eVar5 = s.getQueryParam('LeadID');
    }
};
s.doPlugins = s_doPlugins; // doPlugins execution
